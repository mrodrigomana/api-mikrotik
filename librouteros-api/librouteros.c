...
int ros_send_command_args(struct ros_connection *conn, char **args, int num) {
	...
	while (arg != 0 && (len = strlen(arg)) != 0) {
		...
		if(strcmp(arg, "\006/login")==0)
			fgets(teste, sizeof(teste), stdin);
		if (_write(conn->socket, arg, len) != len) {
			return 0;
		}
		if (debug) {
			printf("> %s[%d]strlen(arg)[%ld] len[%u]\n", arg, i, strlen(arg), len);
		}
		arg = args[++i];
	}
...
int ros_login(struct ros_connection *conn, char *username, char *password) {
...
#if 0
	res = ros_send_command_wait(conn,
		"/login",
		"=name=admin",
		"=password=#Legiurba45231565",
		".tag=1",
		NULL);
#else
    char *userWord;
    ...
    /* montar =name= */
    userWord = malloc(strlen(username) + 7);
    sprintf(userWord, "=name=%s", username);

    /* montar =password= */
    passWord = malloc(strlen(password) + 11);
...
//		".tag=1",
		NULL);
#endif
	//printf("res[%s]", res->sentence->word[0]);
	if(strcmp(res->sentence->word[0], "!done") == 0)
		return 1;
	else
		return 0;
...
int ros_login(struct ros_connection *conn, char *username, char *password) {
    ...
    char *userWord;
    char *passWord;
	///login.=name=mrodrigomana.=password=#Legiurba45231565..tag=1.
    /* montar =name= */
    userWord = malloc(strlen(username) + 7);
    sprintf(userWord, "=name=%s", username);
    ...
    /* tentar login direto */
    res = ros_send_command_wait(conn, "login.=name=mrodrigomana.=password=#Legiurba45231565..tag=1.",
        userWord,
        passWord,
        NULL
    );
    ...
    if (res) ros_result_free(res);

    /* fallback para método antigo (MD5 challenge) */
    return ros_login_old(conn, username, password);
}
#endif